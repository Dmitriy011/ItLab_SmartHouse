...
class ISensors
{
    virtual string generate_one_lexeme() = 0;
...
public:
    ...
    void get_string() override
    {
        ...
        ifstream in("C:\\Users\\Kek\\Desktop\\Total_sensors_data.txt", ios::in); // îêðûâàåì ôàéë äëÿ ÷òåíèÿ
        if (in.is_open())
        {
    ...
    float degree = 0;                   //Ïðîöåíòíîå çíà÷åíèå âûðàæàþùåå ñîñòîÿíèå
    float max_value = 1, min_value = 0; //Ìàêñèìàëüíîå è ìèíèìàëüíîå çíà÷åíèÿ ñîñòîÿíèÿ
    bool on_off = 0;

    virtual void print_info() = 0; //Âûâîä òåêóùåé èíôîðìàöèè îá îáúåêòå
    bool is_on()
    {
        if (on_off == ON)
        {
    ...
    float get_degree() //Ïîëó÷åíèå çíà÷åíèÿ ñîñòîÿíèÿ
    {
        return this->degree;
    };
    void set_degree(float new_degree) //Óñòàíîâêà çíà÷åíèÿ ñîñòîÿíèÿ
    {
        degree = new_degree;
    };

    void set_on_off(bool temp_on_off) //Âêë/âûêë
    {
